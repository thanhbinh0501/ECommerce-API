...
export class ProductBloc {
  ...
  async createProduct({ userId: currentUserId }: CtxReq, req: ProductCreateReq): Promise<number> {
    const { name, description, categoryIds, colorId, romId, price, stock } = req;
    ...
    const categories = await this.productService.getCategoriesByIds(categoryIds);
    if (!categories || categories.length === 0) {
      throw new NotFoundException(`Category with id ${categoryIds} not found`);
    }
  ...
  async updateProduct(id: number, req: ProductCreateReq, currentUserId: number): Promise<void> {
    ...
    if (req.categoryIds) {
      const categories = await this.productService.getCategoriesByIds(req.categoryIds);
      if (!categories || categories.length === 0) {
        throw new NotFoundException(`No categories found for ids ${req.categoryIds}`);
      }

      await this.productService.removeProductCategories(id);
      for (const category of categories) {
        await this.productService.createProductCategory(id, category.id, currentUserId);
      }
    }
